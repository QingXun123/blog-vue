          ...
          <ul ref="chatList" class="chat-list" v-infinite-scroll="chatLoad" style="overflow:auto">
            <li v-for="(oneChatData, index) in chatData" :key="index - 1" class="oneChat" ref="chatItem">
              <div class="myChatFlex"
                   v-if="oneChatData.userId === randomUserId || String(oneChatData.userId) === String(user.userId) || oneChatData.userName === randomUserId">
...
export default {
  ...
  methods: {
    ...
    getUserList() {
      getOnlineUserByRoomId(1).then((response) => {
        const data = response.data;
        if (data.code != 0) {
          this.$message({
            'type': 'error',
    ...
    getOnlineUserCount() {
      getOnlineCountByRoomId(1).then((response) => {
        const data = response.data;
        if (data.code !== 0) {
          this.$message({
            'type': 'error',
